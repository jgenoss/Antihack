/**
 * AntiCheatCore - Memory Patcher Implementation
 * Utility class for safe memory read/write operations
 */

#include "stdafx.h"
#include "../include/internal/MemoryPatcher.h"
...
DWORD MemoryPatcher::SetJg(const LPVOID dwEnterFunction, const LPVOID dwJMPAddress) {
    ...
    btBuf[0] = 0x0F;  // JG rel32 (two-byte opcode)
    btBuf[1] = 0x8F;
...
DWORD MemoryPatcher::SetJa(const LPVOID dwEnterFunction, const LPVOID dwJMPAddress) {
    ...
    btBuf[0] = 0x0F;  // JA rel32 (two-byte opcode)
    btBuf[1] = 0x87;
    memcpy(&btBuf[2], &dwShift, 4);
